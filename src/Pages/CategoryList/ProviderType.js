...
function ProviderType() {
  ...
  const fetchData = async () => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}types?type=provider&category=all`)
      .then(function (res) {
  ...
  useEffect(() => {
    if (!loading && tableRef.current) {
      ...
      $(tableRef.current).DataTable({}); // Reinitialize DataTable
    }
  }, [loading]);
  ...
  const handleClick = (e) => {
    ...
    if (row.Type === "") {
      ...
      axios
        .post(`${process.env.REACT_APP_BASE_URL}types?type=provider`, row)
        .then((res) => {
  ...
  const handleDeleteClick = (id) => {
    axios
      .put(`${process.env.REACT_APP_BASE_URL}types/archive?type=provider`, {
        // ...selectedItem,
        id,
      })
      .then((res) => {
        ...
        axios
          .get(
            `${process.env.REACT_APP_BASE_URL}types?type=provider&category=all`
          )
          .then((res) => {
            // console.log(res.data.data);
            setData(res?.data?.data);
  ...
  const handleUpdateProvider = () => {
    ...
    axios
      .put(`${process.env.REACT_APP_BASE_URL}types?type=provider`, {
        ...selectedItem,
        Type: editedType,
      })
      .then((res) => {
                ...
                <div className="col-12">
                  <div className="card">
                    <div className="card-header d-flex justify-content-between">
                      <h6 className="card-title">Provider Type</h6>
                    </div>
            ...
            <div className="row g-2 clearfix row-deck mt-3">
              <div className="col-lg-12 col-md-12">
                <div className="card mb-4">
                  <div className="card-body">
                    <table id="userList" ref={tableRef} className="table table-hover">
                      <thead>
                        <tr>
                          <th>#</th>
                          <th>S.No.</th>
      ...
      >
        <div class="custom_content-wrapper tw-w-[50%]">
          ...
          <header class="custom_modal-header">
            <h2>Edit Provider Type</h2>
          </header>
          <div className="tw-w-[100%]">
            <div className="col-12">
              <label className="form-label">Edit Provider Type</label>
              <input
                type="text"
                value={editedType}
          ...
          <footer class="custom_modal-footer">
            <button class="action" type="submit" onClick={handleUpdateProvider}>Accept</button>
