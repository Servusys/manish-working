...
function ArchiveProviderType() {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}types?type=provider&category=archive`
      )
  ...
  const restoreClick = (data, id) => {
    // alert('running')
    axios
      .put(`${process.env.REACT_APP_BASE_URL}types/restore?type=provider`, {
        id,
        archiveId: data?.id,
      })
      .then((res) => {
  ...
  const handleClick = (id) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}users/getArchiveHistory?type=provider&productId=${id}`
      )
      .then((res) => {
        console.log(
          "HistoryDATA filtered",
          res.data.message?.filter((i) => {
        ...
        //  ?.filter((i)=>{return i?.RestoreDate==null})
        console.log({ data1 });
                        ...
                        {records?.length &&
                          records?.map((item, i) => (
                            <tr key={i}>
                              ...
                              <td>{item?.Type}</td>
                              <td className="tw-flex tw-gap-x-1">
                                <a
                                  href="#"
...
export default ArchiveProviderType;
