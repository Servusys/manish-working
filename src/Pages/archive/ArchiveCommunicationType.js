...
function ArchiveCommunicationType() {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}types?type=communication&category=archive`
      )
  ...
  const handleRestore = (data, id) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}types/restore?type=communication`,
        {
          id,
          archiveId: data?.id,
        }
  ...
  const handleClick = (id) => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}users/getArchiveHistory?type=communication&productId=${id}`
      )
      .then((res) => {
        console.log(
          "HistoryDATA",
