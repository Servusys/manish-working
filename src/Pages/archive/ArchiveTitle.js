...
function ArchiveTitle() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}types?type=title&category=archive`)
  ...
  const handleClick = (id) => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}users/getArchiveHistory?type=title&productId=${id}`
      )
      .then((res) => {
  ...
  const handleRestore = (data, id) => {
    // alert(id);
    axios
      .put(`${process.env.REACT_APP_BASE_URL}types/restore?type=title`, {
        id,
        archiveId: data?.id,
      })
      .then((res) => {
        ...
        axios
          .get(
            `${process.env.REACT_APP_BASE_URL}types?type=title&category=archive`
          )
          .then((res) => {
