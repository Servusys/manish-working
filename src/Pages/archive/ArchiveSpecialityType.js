...
function ArchiveSpecialityType() {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}types?type=speciality&category=archive`
      )
  ...
  const handleRestore = (data, id) => {
    // alert(id);
    axios
      .put(`${process.env.REACT_APP_BASE_URL}types/restore?type=speciality`, {
        id,
        archiveId: data?.id,
      })
      .then((res) => {
  ...
  const handleClick = (id) => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}users/getArchiveHistory?type=speciality&productId=${id}`
      )
      .then((res) => {
...
export default ArchiveSpecialityType;
