...
function ArchiveUserRole() {
  ...
  const handleFilter = (e) => {
    ...
    const newData = originalRecords.filter((row) => {
      return (
        row.UserRoleName.toLowerCase().includes(inputValue.toLowerCase()) ||
        row.id == +inputValue
      );
    });
    setRecords(newData);
  ...
  const handleRestore = (data, id) => {
    axios
      .put(`${process.env.REACT_APP_BASE_URL}userRole/restore`, {
        id,
        archiveId: data?.id,
      })
      .then((res) => {
                        ...
                        {records?.length &&
                          records?.map((item, i) => (
                            <tr key={i}>
                              ...
                              <td className=" tw-flex tw-gap-x-2 text-center">
                                <a
                                  href="#"
...
export default ArchiveUserRole;
