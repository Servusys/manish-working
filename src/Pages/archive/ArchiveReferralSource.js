...
function ArchiveReferralSource() {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}types?type=referralSource&category=archive`
      )
  ...
  const handleRestore = (data, id) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}types/restore?type=referralSource`,
        {
          id,
          archiveId: data?.id,
        }
  ...
  const handleClick = (id) => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}users/getArchiveHistory?type=referralSource&productId=${id}`
      )
      .then((res) => {
...
export default ArchiveReferralSource;
