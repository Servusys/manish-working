...
function ArchiveSubscription() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}subscription/archive`)
      .then((res) => {
  ...
  const handleClick = (id) => {
    // alert(id);
    axios
      .put(`${process.env.REACT_APP_BASE_URL}subscription/restore`, {
        id,
      })
      .then((res) => {
  ...
  const handleClickData = (id) => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}users/getArchiveHistory?type=Subscription&productId=${id}`
      )
      .then((res) => {
